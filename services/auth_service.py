...
async def register(db: AsyncSession, data: BuyerCreate) -> Buyer:
	...
	new_buyer = await create_buyer(db, data)
	try:
		await db.commit()
	except IntegrityError as exc:
		await db.rollback()
		if "unique" in str(exc.orig).lower() or "buyers_email_key" in str(exc.orig):
			raise HTTPException(
				status_code=status.HTTP_409_CONFLICT,
				detail="Покупатель с таким email уже существует",
			) from exc
...
async def refresh_tokens(db: AsyncSession, raw_token: str) -> TokenResponse:
	...
	buyer = await get_buyer_by_id(db, db_token.buyer_id)
	return await _issue_tokens(db, buyer)
...
async def _issue_tokens(db: AsyncSession, buyer: Buyer) -> TokenResponse:
	...
	result = await db.execute(
		...
		.where(RefreshToken.buyer_id == buyer.id)
